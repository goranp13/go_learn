...
func RunMqttClient() {
	...
	mqttClient := NewClient(
		...
		"",
		"",
		fmt.Sprintf("ssl://%s", "vernemq-vernemq.apps.thingstalk.eu:8883"),
	)
...
func NewClient(ctx log.Interface, id, username, password string, brokers ...string) Client {
	...
	mqttOpts.SetClientID("ypf_dewqfvcd9365daa4-acd6-46b3-b5f9-8abb37128b78eqfcdqwcdq")
	mqttOpts.SetUsername("9365daa4-acd6-46b3-b5f9-8abb37128b78")
	mqttOpts.SetPassword("")
	...
	mqttOpts.SetDefaultPublishHandler(func(client MQTT.Client, msg MQTT.Message) {
		ctx.WithField("free:000", msg).Warn("Received unhandled message")
	})
...
func NewTLSConfig() *tls.Config {
	...
	pemCerts, err := ioutil.ReadFile("./vernemq_ca.pem.pem")
	if err == nil {
	...
	cert, err := tls.LoadX509KeyPair("./vernemq_ca.pem", "./vernemq_ca.pem")
	if err != nil {
		panic(err)
	}
